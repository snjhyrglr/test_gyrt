require "application_system_test_case"

class BatchesTest < ApplicationSystemTestCase
  setup do
    @batch = batches(:one)
  end

  test "visiting the index" do
    visit batches_url
    assert_selector "h1", text: "Batches"
  end

  test "should create batch" do
    visit batches_url
    click_on "New batch"

    fill_in "Cooperative", with: @batch.cooperative_id
    fill_in "Issuing office", with: @batch.issuing_office
    fill_in "Quote no", with: @batch.quote_no
    fill_in "Quote year", with: @batch.quote_year
    fill_in "Total coop sf", with: @batch.total_coop_sf
    fill_in "Total gross prem", with: @batch.total_gross_prem
    fill_in "Total net prem", with: @batch.total_net_prem
    click_on "Create Batch"

    assert_text "Batch was successfully created"
    click_on "Back"
  end

  test "should update Batch" do
    visit batch_url(@batch)
    click_on "Edit this batch", match: :first

    fill_in "Cooperative", with: @batch.cooperative_id
    fill_in "Issuing office", with: @batch.issuing_office
    fill_in "Quote no", with: @batch.quote_no
    fill_in "Quote year", with: @batch.quote_year
    fill_in "Total coop sf", with: @batch.total_coop_sf
    fill_in "Total gross prem", with: @batch.total_gross_prem
    fill_in "Total net prem", with: @batch.total_net_prem
    click_on "Update Batch"

    assert_text "Batch was successfully updated"
    click_on "Back"
  end

  test "should destroy Batch" do
    visit batch_url(@batch)
    click_on "Destroy this batch", match: :first

    assert_text "Batch was successfully destroyed"
  end
end
